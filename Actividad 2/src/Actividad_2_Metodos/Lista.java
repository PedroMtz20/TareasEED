...
public class Lista<E> {
    ...
    //Metodo de exist, busca por dato entre los nodos
    public Nodo buscarData(String data) throws Exception{
        Nodo resultado = null;
        Nodo nodoActual = this.firstNodo;
        boolean encontrado = false;
        //Hace una comprobacion en cada indice para ver si encuentra el dato
        for (int i = 1; (nodoActual != null && !encontrado); i++){
            ...
            //En caso de ser encontrado da el next
            nodoActual = nodoActual.getNext();
        }
        return resultado;
    }
    
    //Metodo utilizado para buscar datos por medio de lo que esta como data
    public boolean buscarDataString(String data) throws Exception{
        Nodo resultado = null;
        Nodo nodoActual = this.firstNodo;
        boolean encontrado = false;
        //Se busca dentro de los nodos una vez
        for (int i = 1; (nodoActual != null && !encontrado); i++){
            //Si lo encuentra se cambia el encontrado el ture y se imprime
            if(nodoActual.getData() == data){
                encontrado = true;
                resultado = nodoActual;
                System.out.println("El dato " + data + " esta en la posicion: " + i);
            }
            //En caso de ser encontrado y que se compare de manera correcta se cambia encontrado a true
    ...
    //Metodo como el de insertar pero que actualiza datos.
